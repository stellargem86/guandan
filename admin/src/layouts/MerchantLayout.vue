...
const pageTitle = computed(() => (route.meta.title as string) || '商户门户')

const navItems = [
  { path: '/merchant-portal/dashboard', label: '仪表盘', icon: '📊' },
  { path: '/merchant-portal/verify', label: '核销管理', icon: '✅' },
  { path: '/merchant-portal/packages', label: '套餐管理', icon: '🎁' },
  { path: '/merchant-portal/settlement', label: '结算提现', icon: '💳' },
]
      ...
      <div class="px-3 py-4 border-t border-dark-600">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 bg-dark-600 rounded-full flex items-center justify-center text-xs text-gold-400 flex-shrink-0">
            {{ authStore.user?.nickname?.charAt(0) || 'M' }}
          </div>
          <div v-if="!appStore.sidebarCollapsed" class="flex-1 min-w-0">
            <p class="text-sm text-gray-200 truncate">{{ authStore.user?.nickname || '商户' }}</p>
            <p class="text-xs text-gray-500">商户管理</p>
          </div>
        </div>
      </div>
    </aside>
